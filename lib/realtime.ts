  ...
  for (const id of ids) {
    ...
    if (!l) {
      if (p) {
        ...
        // Only keep if we're already tracking it locally.
...
export function mergeRealtimeRequest(current: HoistRequest[], payload: RequestRealtimePayload) {
  ...
  // This prevents completed/cancelled requests from reappearing after refresh
  // when the Realtime event for the status change arrives.
