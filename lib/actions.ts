...
export async function adminDeactivateOperatorTablet(projectId: string, elevatorId: string) {
  ...
  if (manualFullError && !isMissingElevatorManualFullColumn(manualFullError)) {
    // Non-critical: don't block deactivation for this
  }
...
function isLegalTransition(from: RequestStatus, to: RequestStatus): boolean {
  ...
  if (!allowed) return false; // terminal status
...
export async function updateRequestStatus(
  ...
  // Validate status transition is legal (prevent backward/invalid transitions)
